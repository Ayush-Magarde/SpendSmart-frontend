...
export class ExpenseComponent implements OnInit {
  ...
  isDefaultCategory(name: string): boolean {
    return this.expenseCategories().find(c => c.name === name)?.isDefault || false;
  }
  ...
  deleteExpense(expense: Expense) {
    ...
    this.notifService.confirm(
      'Delete Expense',
      `Delete this ₹${expense.amount} expense (${expense.category})?`,
      () => {
        this.deletingId.set(expense.id!);
        this.expenseService.deleteExpense(expense.id!).subscribe({
          next: () => {
            this.deletingId.set(null);
            this.notifService.success('Expense Deleted', 'The transaction has been removed.');
          },
          error: () => {
  ...
  onSubmit() {
    ...
    this.expenseService.addExpense(newExpense).subscribe({
      next: () => {
        this.notifService.success('Expense Added', `₹${newExpense.amount} added to ${newExpense.category}`);
